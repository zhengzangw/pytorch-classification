...
class Compose(object):
    ...
    def __call__(self, img, masks):
        ...
        if self.PIL2Numpy:
            img = np.array(img)
            for i, mask in enumerate(masks):
                masks[i] = np.array(mask, dtype=dtypes[i])
...
# ========
# Augmentation not involves mask
# ========
...
# ========
# Augmentation involves mask
# !TODO: now only support rcrop, rsize & hflip
# ========
...
class RandomCrop(object):
    ...
    def __call__(self, img, masks):
        if self.padding > 0:
            ...
            for i, mask in enumerate(masks):
                masks[i] = ImageOps.expand(mask, border=self.padding, fill=0)
        ...
        if w < tw or h < th:
            for i, mask in enumerate(masks):
                masks[i] = mask.resize((tw, th), Image.NEAREST)
            return (
        ...
        for i, mask in enumerate(masks):
            masks[i] = mask.crop((x1, y1, x1 + tw, y1 + th))
        return (
...
class RandomSized(object):
    ...
    def __call__(self, img, masks):
        ...
        for i, mask in enumerate(masks):
            masks[i] = mask.resize((w, h), Image.NEAREST)
...
def get_composed_augmentations(aug_dict):
    if aug_dict is None:
        log.info("Using No Augmentations")
        return None

    augmentations = []
    for aug_key, aug_param in aug_dict.items():
        augmentations.append(key2aug[aug_key](aug_param))
        log.info("Using {} aug with params {}".format(aug_key, aug_param))
